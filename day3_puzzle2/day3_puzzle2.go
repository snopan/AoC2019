	...
	for _, intv_1 := range path_1 {
		...
		for _, intv_2 := range path_2 {
			...
			if (found) {
				...
				extra_x1 := math.Abs(intersect.x - intv_1.start.x)
				extra_y1 := math.Abs(intersect.y - intv_1.start.y)
				extra_x2 := math.Abs(intersect.x - intv_2.start.x)
				extra_y2 := math.Abs(intersect.y - intv_2.start.y)

				total_steps := float64(steps_1) + float64(steps_2) + extra_x1 + extra_y1 + extra_x2 + extra_y2
				fmt.Println(total_steps)
			}

			steps_2 += intv_2.steps
		}

		steps_1 +=  intv_1.steps
		steps_2 = 0
	}
}
